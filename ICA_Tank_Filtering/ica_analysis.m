...
% Plot ALL ICA components in groups of 16 using a 4x4 layout
    ...
    for figIndex = 1:numFigures
        ...
        sgtitle([array_name ': ICA Components ' ...
                 num2str((figIndex-1)*numPerFigure + 1) '-' ...
                 num2str(min(figIndex*numPerFigure, numComponentsToExtract))], ...
    ...
    colors = lines(length(componentsToUse)); % Generate distinct colors
    
    for i = 1:length(componentsToUse)
        subplot(length(componentsToUse),1,i);
        plot(To, icasig(componentsToUse(i),:), 'LineWidth', 2, 'Color', colors(i,:));
        title(['Selected IC ' num2str(componentsToUse(i))], ...
               'FontSize', 12, 'FontWeight', 'bold');
        xlabel('Time (s)', 'FontSize', 10);
        ylabel('Amplitude', 'FontSize', 10);
        set(gca, 'FontSize', 9, 'Color', 'white');
        box on;
    end
    
    sgtitle([array_name ': Selected ICA Components'], ...
    ...
    subplot(2,1,1);
    plot(To, Data_matrix(1,:), 'LineWidth', 1.5, 'Color', [0.8 0.2 0.2]);
    title(['Original Signal - Electrode ' num2str(electrode_indices(1))], ...
           'FontSize', 12, 'FontWeight', 'bold');
    xlabel('Time (s)', 'FontSize', 10);
    ylabel('Amplitude', 'FontSize', 10);
    xlim([0 1]);
    ...
    subplot(2,1,2);
    plot(To, Data_reconstructed(1,:), 'LineWidth', 1.5, 'Color', [0.2 0.6 0.2]);
    ...
    sgtitle([array_name ': Signal Comparison'], ...
             'FontSize', 16, 'FontWeight', 'bold');
