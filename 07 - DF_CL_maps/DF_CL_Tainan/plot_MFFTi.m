function plot_MFFTi(MFFTi, plotTitle, cLimits, plotMode)
    % Function: plot_MFFTi
    % Description:
    %   This function plots the MFFTi matrix using a color-coded representation
    ...
    function output_txt = dataCursorCallback(~, event_obj)
        ...
        x_coord = pos(1);
        y_coord = pos(2);
        value = MFFTi(y_coord, x_coord); % Note: MFFTi uses (y, x) convention

        output_txt = {
            ['X: ', num2str(x_coord)], ...
            ['Y: ', num2str(y_coord)], ...
            ['Frequency (Hz): ', num2str(value)]
            };
    end
end
