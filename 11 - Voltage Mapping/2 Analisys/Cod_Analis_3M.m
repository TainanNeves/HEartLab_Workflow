...
[path, ~, ~] = fileparts(fullfilename); path_parts = strsplit(path, filesep);
folderName = path_parts{end};

%% PARTE 3 - CONFIGURAÇÕES INICIAIS

T = {'MEA1-LA', 'MEA2-RA', 'MEA3-V'}; MEAs = {1:16, 17:32, 81:96}; 

Fs = DATA.D_EL.Header.sample_rate;

percentage_results_m = struct('MEA1', [], 'MEA2', [], 'MEA3', []);
percentage_results_a = struct('MEA1', [], 'MEA2', [], 'MEA3', []);
...
for mea_idx = 1:length(MEAs)
    es = MEAs{mea_idx}; num_eletrodos = length(es); num_figuras = ceil(num_eletrodos / 2);
    for fig_idx = 1:num_figuras
        ...
        for subplot_idx = 1:2
            ...
            e = es(eletrodo_idx); SS = DATA.D_EL.Data(e, :); subplot(2, 1, subplot_idx);
            plot((1:length(SS)) / Fs, SS / 1000); xlabel('Tempo (s)'); ylabel('Amplitude (mV)');
...
for i = 1:3
    subplot(3,1,i); e = eletrodos_analise(i);
    if e <= 16
        mea_idx = 1;
    elseif e <= 32
        mea_idx = 2;
    ...
    sinal = DATA.D_EL.Data(e, :)/1000; tempo = (0:length(sinal)-1)/Fs; plot(tempo, sinal); 
    xlim(janela_tempo);  title(['Eletrodo ' num2str(e) ' - ' T{mea_idx}], 'FontSize', 11);
    xlabel('Tempo (s)'); ylabel('mV'); grid on; hold on;
end
...
savefig(fullfile(output_folder, [folderName '_F2_RA_LA_V_Marcados_.fig'])); disp('Finalizado!');

%% PARTE 7 - DETERMINAÇÃO DA JANELA DE TRABALHO

eletrodo_ref = MEAs{1}(1); sinal_ref = DATA.D_EL.Data(eletrodo_ref, :); tempo_total = (0:length(sinal_ref)-1)/Fs;
figure('Name', 'Seleção da Janela de Análise', 'NumberTitle', 'off', 'Position', [100 100 900 500]);
plot(tempo_total, sinal_ref / 1000); xlabel('Tempo (s)'); ylabel('Amplitude (mV)');
title(['Sinal de Referência (Eletrodo ' num2str(eletrodo_ref) ') - Selecione a janela de análise']); grid on;

disp('Selecione a janela de análise no gráfico (clique em dois pontos)');
[x, ~] = ginput(2); janela_tempo = sort(x);
...
janela_amostras = round(janela_tempo * Fs); janela_amostras(1) = max(1, janela_amostras(1));
janela_amostras(2) = min(length(sinal_ref), janela_amostras(2));
janela.tempo_inicio = janela_tempo(1);      janela.tempo_fim = janela_tempo(2);
janela.amostra_inicio = janela_amostras(1); janela.amostra_fim = janela_amostras(2);
...
percentage_results_m = struct(...
    ...
    'MEA2', struct('Amplitudes', cell(1,16), 'Ativacao', cell(1,16), 'Recuperacao', cell(1,16), ...
                  'Tempos_Ativacao', cell(1,16), 'Tempos_Recuperacao', cell(1,16)), ...
    'MEA3', struct('Amplitudes', cell(1,15), 'Ativacao', cell(1,15), 'Recuperacao', cell(1,15), ...
                  'Tempos_Ativacao', cell(1,15), 'Tempos_Recuperacao', cell(1,15)));

style = struct(...
    'MEA1', struct('color', 'b', 'marker_ativ', 'go', 'marker_rec', 'ro'), ...
    'MEA2', struct('color', 'r', 'marker_ativ', 'g^', 'marker_rec', 'r^'), ...
    'MEA3', struct('color', 'g', 'marker_ativ', 'gs', 'marker_rec', 'rs'));

mea_list = {'MEA1', 'MEA2', 'MEA3'}; 
mea_eletrodos = {MEA1, MEA2, MEA3};
    ...
    for e_idx = 1:length(eletrodos)
        ...
        while ~done
            ...
            if strcmp(proximo_passo, 'ativacao') && strcmp(button, 'normal')
                ...
                plot(x, y / 1000, style.(current_mea).marker_ativ, 'MarkerSize', 10, 'LineWidth', 1.5);
                text(x, y / 1000, sprintf(' Ativ: %.1f mV', y / 1000), 'Color', 'k', 'FontSize', 9);
            ...
            elseif strcmp(proximo_passo, 'recuperacao') && strcmp(button, 'alt')
                if ~isempty(pares(end).ativacao_val)
                    ...
                    plot(x, y / 1000, style.(current_mea).marker_rec, 'MarkerSize', 10, 'LineWidth', 1.5);
                    text(x, y / 1000, sprintf(' Rec: %.1f mV', y / 1000), 'Color', 'k', 'FontSize', 9);
                    proximo_passo = 'ativacao';
...
save(fullfile(output_folder, [folderName '_manual_amplitudes.mat']), ...
     'percentage_results_m', 'janela', '-v7.3');
disp('Medições manuais salvas com sucesso!');
...
MEA_all = {MEA1, MEA2, MEA3}; MEA_names = {'MEA1-LA', 'MEA2-RA', 'MEA3-V'};

for m = 1:length(MEA_all)
    current_mea = ['MEA' num2str(m)]; MEA = [MEA_all{m}{:}];
    ...
    num_eletrodos = length(MEA); num_figuras = ceil(num_eletrodos / 2);
    ...
    for fig_idx = 1:num_figuras
        ...
        sgtitle(sprintf('%s - Fig %d - %s', MEA_names{m}, fig_idx, folderName));
        savefig(fullfile(output_folder, [folderName '_F3_Auto_Peak-to-Peak_' MEA_names{m} '_Fig' num2str(fig_idx) '.fig']));
    end
end

save(fullfile(output_folder, [folderName '_auto_amplitudes.mat']), ...
     'percentage_results_a', 'janela', '-v7.3');
...
for m = 1:length(MEA_all)
    ...
    for fig_idx = 1:num_figuras
        ...
        sgtitle(sprintf('%s - Fig %d - %s | Janela: %.1fs a %.1fs', ...
               MEA_names{m}, fig_idx, folderName, ...
               janela.tempo_inicio, janela.tempo_fim));
        savefig(fullfile(output_folder, [folderName '_F4_HR_' MEA_names{m} '_Fig' num2str(fig_idx) '.fig']));
    end
    HR_str.(current_mea) = HR_values;
end

save(fullfile(output_folder, [folderName '_heart_rate_results.mat']), ...
     'HR_str', 'janela', '-v7.3');

%% PARTE 11 - ESTATÍSTICA DAS FREQUÊNCIAS DE ATIVAÇÃO E CYCLE LENGTHS

MEA_names = {'MEA1-LA', 'MEA2-RA', 'MEA3-V'}; valid_AF = struct();

for m = 1:length(MEA_names)
    current_mea = ['MEA' num2str(m)];
...
plot_data = []; group_labels = [];
for m = 1:length(MEA_names)
    current_data = valid_AF.(['MEA' num2str(m)]);
    plot_data = [plot_data; current_data'];
    group_labels = [group_labels; repmat(MEA_names(m), length(current_data), 1)];
end
...
bar_width = 0.8; x_pos = 1:length(MEA_names);
...
set(gca, 'XTick', x_pos, 'XTickLabel', MEA_names, 'FontSize', 11);
ylabel('Frequência de Ativação (Hz)', 'FontSize', 12);
...
mea_names = {'MEA1-LA', 'MEA2-RA', 'MEA3-V'}; colors = [0 0.4470 0.7410; 0.8500 0.3250 0.0980]; bar_width = 0.35;
auto_data = {[percentage_results_a.MEA1.Amplitudes]/1000, [percentage_results_a.MEA2.Amplitudes]/1000, [percentage_results_a.MEA3.Amplitudes]/1000};
manual_data = {[percentage_results_m.MEA1.Amplitudes]/1000, [percentage_results_m.MEA2.Amplitudes]/1000, [percentage_results_m.MEA3.Amplitudes]/1000};
auto_stats = struct(); manual_stats = struct();

for m = 1:3
    current_auto = auto_data{m}(~isnan(auto_data{m}));
    current_manual = manual_data{m}(~isnan(manual_data{m}));  
    auto_stats(m).MEA = mea_names{m};
    ...
    manual_stats(m).MEA = mea_names{m};
...
figure('Position', [100, 100, 800, 500], 'Name', 'Comparação de Medianas das Amplitudes');
...
set(gca, 'XTick', x, 'XTickLabel', mea_names);
ylabel('Amplitude (mV)');
...
figure('Position', [100, 100, 900, 600], 'Name', 'Distribuição Combinada das Amplitudes');
combined_data = cell(1,6);
for m = 1:3
...
for m = 1:3
    ...
    labels = [labels sprintf('%s-Auto', mea_names{m}) sprintf('%s-Manual', mea_names{m})];
end

max_len = max(cellfun(@length, combined_data));
plot_data = NaN(max_len, 6);
for i = 1:6
...
for m = 1:3
    disp(' ');
    disp(['ESTATÍSTICAS PARA ' mea_names{m} ':']);
    disp('-----------------------------------');
...
savefig(fullfile(output_folder, [folderName 'F8_Comparacao_Medianas_Amplitudes.fig']));
savefig(fullfile(output_folder, [folderName 'F9_Boxplots_Amplitudes.fig']));
savefig(fullfile(output_folder, [folderName 'F10_Distribuicao_Amplitudes.fig']));
...
writetable(comparison_table, fullfile(output_folder, [folderName '_Comparacao_Amplitudes.csv']), 'WriteRowNames', true);

amplitude_results = struct(...
    'auto_stats', auto_stats, ...
    'manual_stats', manual_stats);

save(fullfile(output_folder, [folderName '_amplitude_results.mat']), 'amplitude_results', '-v7.3');

%% PARTE 13 - VISÃO GERAL DA MEDIANA DAS AMPLITUDES PICO-A-PICO

MEAs = {[MEA1{:}], [MEA2{:}], [MEA3{:}]}; MEA_nomes = {'MEA1-LA', 'MEA2-RA', 'MEA3-V'};
cores = [0 0.5 0.8; 0.8 0.3 0.2];

for mea_idx = 1:3
    current_mea = ['MEA' num2str(mea_idx)];
    fig = figure('Position', [100, 100, 1200, 800], 'Color', 'w', 'Name', ['Comparação de Medianas - ' MEA_nomes{mea_idx}], 'NumberTitle', 'off');
    sgtitle(['Comparação de Medianas Manual (M) e Automático (A) - ' MEA_nomes{mea_idx} ' | ' folderName], 'FontSize', 14, 'FontWeight', 'bold');
    eletrodos = MEAs{mea_idx}; num_eletrodos = length(eletrodos); num_linhas = ceil(num_eletrodos/4);
    for e = 1:num_eletrodos
        subplot(num_linhas, 4, e); hold on; e_idx = find(eletrodos(e) == MEAs{mea_idx});
        ...
        title(['E' num2str(eletrodos(e))], 'FontSize', 10); ylabel('Amplitude (mV)', 'FontSize', 9);
        xlim([0 2]); set(gca, 'XTick', [], 'Box', 'on', 'FontSize', 8); grid on;
    ...
    savefig(fig, fullfile(output_folder, [folderName '_F11_Comparacao_Medianas_' MEA_nomes{mea_idx} '.fig']));
...
activation_frequency = struct('MEA1', HR_str.MEA1, 'MEA2', HR_str.MEA2, 'MEA3', HR_str.MEA3);

mean_HR = struct();
mean_HR.MEA1 = mean(HR_str.MEA1(~isnan(HR_str.MEA1)));
mean_HR.MEA2 = mean(HR_str.MEA2(~isnan(HR_str.MEA2)));
mean_HR.MEA3 = mean(HR_str.MEA3(~isnan(HR_str.MEA3)));

mean_activation_frequency = struct('MEA1', mean_HR.MEA1, 'MEA2', mean_HR.MEA2, 'MEA3', mean_HR.MEA3);

peak_to_peak_amplitudes = struct('MEA1', [], 'MEA2', [], 'MEA3', []);
...
mean_amplitude_per_MEA.MEA3 = mean([percentage_results_m.MEA3.Amplitudes]/1000, 'omitnan');

mean_peak_to_peak_amplitudes = struct(...
    'MEA1', mean_amplitude_per_MEA.MEA1, ...
    'MEA2', mean_amplitude_per_MEA.MEA2, ...
    'MEA3', mean_amplitude_per_MEA.MEA3);
