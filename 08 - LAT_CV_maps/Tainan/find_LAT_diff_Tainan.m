function LAT_E = find_LAT_diff_Tainan(Data, Fsampling, WinStartIdx, WinEndIdx, case_name, x_coord, y_coord, debug)
    ...
    if debug >= 1
        ...
        % Plot all detected peaks (NO LEGENDS)
        if ~isempty(locs_max)
        ...
        % Plot global max/min
        plot(window_indices(pos_max_global), dvdt_analysis(pos_max_global), 'bs', 'MarkerSize', 8);
        plot(window_indices(pos_min_global), dvdt_analysis(pos_min_global), 'bd', 'MarkerSize', 8);
        
        % Plot edge exclusion zones
        plot([WinStartIdx+edgeExclusion, WinStartIdx+edgeExclusion], ylim, 'g--', 'LineWidth', 0.5);
        plot([WinEndIdx-edgeExclusion, WinEndIdx-edgeExclusion], ylim, 'g--', 'LineWidth', 0.5);
        
        title('dV/dt in Analysis Window (Red: selected, Blue: global, Green: local, Magenta: abs)');
        xlabel('Sample Index');
        ylabel('dV/dt');
        grid on;
        hold off;
