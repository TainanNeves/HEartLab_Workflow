function [headerInfo, imgOut] = Norpix2MATLAB_MarksMod(fileName, showImg, frames)
...
%    FRAMES : if frames =0 read all frames, othewise read the specified
%    number
%    OUTPUTS:
...
if fid==-1
    display('The specified filename does not exist')
    return
end
...
if nargout > 1
    ...
    if frames==0
        ...
        numFramesToProcess = frames;
    end
    if headerInfo.ImageBitDepth == 8
       BitDepth = 'uint8';
    end
    if headerInfo.ImageBitDepth == 16
       BitDepth = 'uint16';
    end
    
  % To allocate a MATLAB array for the processed frames, use this:
    imgOut = zeros(headerInfo.ImageHeight,headerInfo.ImageWidth,numFramesToProcess,BitDepth);
    ...
    for i = 1:numFramesToProcess
        display(['Reading Image Frame ', num2str(i)])
        ...
        switch headerInfo.ImageFormat
            ...
            case 'Monochrome'
                ...
                Image = reshape(UYVYcolVec,headerInfo.ImageWidth,headerInfo.ImageHeight)';
                if showImg
                    imagesc(Image);colormap(gray); drawnow
