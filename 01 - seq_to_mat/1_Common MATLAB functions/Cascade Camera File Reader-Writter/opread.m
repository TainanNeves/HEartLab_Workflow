...
%       .skipping      - number of skipped frames, = 2 read every second,
% 
...
% options.skipping added by Ilija Uzelac, 07-2018
...
if (options.skipping < 1 || options.skipping > h.frames)
	error('options.skipping is out of bounds [1, %d].',h.frames);
end

disp(sprintf('Reading video range [Y=%d:%d, X=%d:%d, frame=%d:%d, skipping=%d]', ...
	options.fromY, options.toY, options.fromX, options.toX, ...
	options.fromFrame, options.toFrame, options.skipping));
...
v = zeros(options.toY-options.fromY+1, options.toX-options.fromX+1, ...
		  floor((options.toFrame-options.fromFrame+1)/options.skipping), options.precision);

N=1;
sk = options.skipping; % for skipping counting	  
fseek(fid, h.blockSize*(options.fromFrame-1), 'cof'); % skip first blocks
for loop = 1:(options.toFrame-options.fromFrame+1)% loop how many frames to load    
    ...
    if sk == options.skipping
        v(:,:,N) = cast(image, options.precision); % Write the result as the next frame in the data matrix.
       sk = 0;
       N=N+1;
    end
    sk = sk+1;
